...
3. 程序总结：概括整个程序的主要功能以及实现方式，并推测程序的具体用途，给出类似软件名称。  

C. 具体要求：  
- 使用函数名称和调用关系进行模块划分，同时结合函数的调用链以及命名模式，辅助以其他合理线索确定模块。  
- 识别关键函数（如程序主入口函数、重要逻辑处理函数），并说明其作用。  
- 整理调用图，形成清晰的模块功能分层结构。  
- 模块划分应尽量合理，避免过度或不足划分。
- 输出结果须为json格式的字符串，无需添加任何额外说明或标记。
...
D. 输出模板：
{
    ...
    "程序功能总结": "程序整体功能的概述，包含主要功能模块及其作用。该程序是一个用于xx的软件，入口为xx函数，类似软件有xxx"
}
...
- 识别关键函数（如程序主入口函数、重要逻辑处理函数），并说明其作用。  
- 整理调用图，形成清晰的模块功能分层结构。  
- 模块划分应尽量合理，避免过度或不足划分，应该把模块所有函数都进行列举。
- 输出结果须为json格式的字符串，无需添加任何额外说明或标记。
...
示例输出：  
{
    ...
    "程序功能总结": "该程序主要功能是对文本进行解码，是一个包含初始化、数据处理和清理模块的典型程序。入口为 `main (0x401000)`，按序调用各模块完成整个流程。经过主要函数功能的推断，我确信它是一个用于文本解码的工具程序。"
}


请根据以上说明分析输入数据，并用中文输出结果."""